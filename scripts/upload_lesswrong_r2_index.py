...
def main() -> None:
    ...
    body = json.dumps(index, indent=2 if args.pretty else None, separators=None if args.pretty else (",", ":")) + "\n"
...
def build_index(
    *,
    ...
    for row in load_success_rows(state_db):
        post = post_from_row(row, cache_dir=cache_dir, prefix=prefix, verify_files=verify_files)
        if not post:
            skipped_missing += 1
            continue
    ...
    for day in sorted(dates.keys(), reverse=True):
        ...
        sorted_dates[day] = {"count": len(posts), "posts": posts}
    ...
    return {
        ...
        "total_posts": sum(day["count"] for day in sorted_dates.values()),
        "dates": sorted_dates,
    }
